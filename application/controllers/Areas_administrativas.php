...
class Areas_administrativas extends LoggedInController {
  ...
  function index()
  {
    $data['header']['title'] = 'Areas Administrativas';
    $this->load->view('areas_administrativas', $data);
    /* Luego listar a que cargos pertenece cada usuario */
  }

  function formulario_agregar()
  {
    $data['header']['title'] = 'Agregar Areas Administrativas';
    $data['escuelas'] = $this->MEscuelas->getEscuelas();
    $data['personal'] = $this->MPersonal->getPersonal();
    $this->load->view('areas_administrativas_formulario', $data);
  }
 ...
 function agregar()
  ...
  function modificar()
  {
    ...
    if($this->form_validation->run() == FALSE){
      ...
      $id = $this->MEscuelas->update($this->input->post('escuelas_id'), $this->input->post('escuelas_nombre'), $this->input->post('escuelas_facultad'));
  ...
  function ajax()
  {
    $table = 'v_areas_administrativas';
    $primaryKey = 'id';
    $columns = array(
      array( 'db' => 'id',       'dt' => 0 ),
      array( 'db' => 'nombre',   'dt' => 1 ),
      array( 'db' => 'escuela',   'dt' => 2 ),
      array( 'db' => 'jefe',   'dt' => 3 ),
      array(
        'db' => 'id', 
        'dt' => 4, 
        'formatter' => function( $d, $row ) {
          return "<a href=\"/areas_administrativas/modificar_formulario/{$row['id']}\">Modificar</a>";
        }
      ),
      array(
        'db' => 'id', 
        'dt' => 5, 
        'formatter' => function( $d, $row ) {
          return "<a href=\"/areas_administrativas/eliminar/{$row['id']}\">Eliminar</a>";
        }
      )
    );
    $sql_details = array(
